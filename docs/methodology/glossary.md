...
### Sync rank
<definition>A measure of ongoing cohesion strength between two or more tickers: how tightly bonded they are to each other over a sustained period. Driven by all three sync types. Applied at two levels: (a) pairwise — the strength of the bond between two specific tickers; (b) aggregate — the cohesion of a candidate net or theme across all its members. Not a price-move predictor — high sync rank indicates the tickers are currently moving closely together; it says nothing about timing or direction of any subsequent move.</definition>
...
<definition>A sync signal in which two or more tickers show sharp price/volume spikes within ±2–3 days of each other in the same direction. Primary for pair activation and event-based confirmation.</definition>
