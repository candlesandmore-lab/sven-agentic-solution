...
### US-015 — Narrative-Cluster Discovery Surface (UPDATED)

**Actor:** `<rule strictness="hard">` human trader `</rule>`

**Action:** `<rule strictness="hard">` When I review narrative findings
drawn from earnings transcripts. `</rule>`

**Instrument:** `<rule strictness="hard">` Technical Trader Solution `</rule>`

**Outcome:** `<rule strictness="hard">` I see tickers grouped into clusters of
...
to a theme. For each cluster I can see which narrative ties it
together, and how its frequency of mention or company breadth (number of
companies in the cluster) is trending.
