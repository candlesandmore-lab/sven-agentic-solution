...
**Exploited behavior:** `<observation>` Exploits MB-006 (sync rank) and
MB-023 (performance dispersion among a theme's member tickers). Performance
dispersion carries opposite implications depending on the prevailing market
regime. `</observation>`
<!-- src: belief/correlation-framework.md L282-288 -->

**Trigger conditions:** `<rule strictness="hard">` (a) In a discretionary
assessment of a rising market environment, wide performance dispersion
within a high-sync theme is read as a tradeable-laggard opportunity. (b) In
a discretionary assessment of a non-rising market environment, the same wide
performance dispersion is read as a fragility warning — a loosely bonded
theme is more likely to break apart than for laggards to catch up. `</rule>`
<!-- src: belief/correlation-framework.md L282-288 -->
...
**Trigger conditions:** `<rule strictness="hard">` (a) High sync rank +
confirmed connection → setups with lower-quality pre-pivot price and volume
action become acceptable, while the pivot level itself remains unchanged.
(b) Lower sync rank + hypothesized (unconfirmed) connection → only the
highest-quality setups are acted on. `</rule>`
<!-- src: belief/correlation-framework.md L373-378 -->
...
shows a confirmed break out of consolidation—one of multiple discretionary
setup types applicable to this strategy, not the sole valid structure—an
event unrelated to the theme's bonding-lifecycle state. Each subsequent
confirmed mover carries
...
(MB-006's Example field was briefly open per Rule 15 during this session's
edits — trader confirmed 2026-07-07 it should be "None" rather than
supplying a replacement; resolved, see market-behavior.md v0.8.)

## Change log

- v0.8 (2026-07-07) — trader-peer review session corrections: STR-003
  re-grounded from MB-006+MB-007 to MB-006+MB-023 (new dedicated dispersion
  entry — dispersion and cohesion looseness are distinct concepts), and
  Trigger conditions reworded from "broadly rising market" / "normal market"
  to explicit discretionary-assessment framing for both regimes; STR-008
  Trigger condition (a) reworded to specify that sync rank lowers the
  pre-pivot price/volume quality bar, not the pivot level itself, resolving
  friction with STR-009's strict break-confirmation requirement; STR-009
  Exploited behavior reworded to scope the confirmed-breakout entry as one
  of multiple discretionary setup types, not the sole valid structure.
