...
spike day is visually discrete and clearly above the recent average. `</observation>`
...
spikes across two or more tickers, within a ±2–3 day window, with visually
discrete above-average volume on the spike day. `</observation>`
<!-- src: belief/correlation-framework.md L27-32 -->
...
**Phenomenon:** `<observation>` Sync rank is driven by all three sync types
(MB-001, MB-002, MB-003). Sync rank decays over time rather than
accumulating indefinitely; the trajectory of sync rank (increasing,
decreasing, steady) carries observational weight. `</observation>`
<!-- src: trader-corrected, 2026-07-07 (review session with trading peer);
supersedes original src: belief/correlation-framework.md L84-86, which
stated resemblance-primacy — trader identified this as inaccurate and
contradicting the corrected Glossary Sync rank / Type 1 definitions -->
...
**Observable indicators:** `<observation>` The current sync rank value
reflects recent contributions from all three sync types. A rising sync rank
trajectory indicates increasing bonding evidence; a falling trajectory
indicates decay of that evidence. `</observation>`
<!-- src: trader-corrected, 2026-07-07 (review session with trading peer);
supersedes original src: belief/correlation-framework.md L90-91 -->
...
**Example:** `<example>` None `</example>`
<!-- src: trader-confirmed 2026-07-07 — prior example (quantum computing
tickers driven by resemblance alone) removed per Rule 16 (no-silent-drop,
2026-07-07 review) as it illustrated the repealed resemblance-primacy
claim; trader declined to supply a replacement -->
...
### MB-009 — Float and Institutional Dollar-Flow Effect on Sync Signal Observability

**Phenomenon:** `<observation>` When institutional buying or selling
represents a significant ratio of a ticker's existing float multiplied by
its price, sync signals (spikes, resemblance) become more pronounced and
visually observable above baseline. This effect is most commonly seen in
lower-float tickers, though low float is not strictly synonymous with low
market capitalization. `</observation>`
<!-- src: trader-corrected, 2026-07-07 (review session with trading peer);
supersedes original src: belief/correlation-framework.md L150-154, which
stated a market-cap-based mechanism — trader identified float-relative
institutional dollar flow as the actual mechanical driver -->
...
**Observable indicators:** `<observation>` Pronounced, highly observable
sync signals in tickers where the dollar volume of institutional activity is
large relative to the ticker's total float value. `</observation>`
<!-- src: trader-corrected, 2026-07-07 (review session with trading peer);
supersedes original src: belief/correlation-framework.md L150-154 -->

**Scope caveat:** `<out-of-scope>` Does not imply a uniform detection
threshold applies across all float sizes; observability is relative to the
specific ticker's baseline behavior. `</out-of-scope>`
<!-- src: trader-corrected, 2026-07-07 -->
<!-- src: belief/correlation-framework.md L153-154 (origin anchor) -->
...
simultaneously exhibit coiling or compression behavior on the 30- to
60-minute timeframe. This is a distinct timeframe and pattern concept from
daily-bar sync, and can be active or inactive independent of it.
`</observation>`
...
coiling in parallel on the 30- to 60-minute timeframe, while other themes
and the broader market do not share that characteristic. `</observation>`
<!-- src: belief/correlation-framework.md L194-200 -->
...
### MB-023 — Performance Dispersion Among Group Members

**Phenomenon:** `<observation>` Performance dispersion — the spread in price
performance across the members of a bonded group or theme over the same
window — varies over time and is itself an observable property, distinct
from sync rank (movement cohesion, MB-006) and performance breadth (the
fraction of members moving together, MB-012). `</observation>`
...
**Observable indicators:** `<observation>` Measurable spread in price/return
performance across a group or theme's member tickers over the same
observation window; can be narrow (tight performance) or wide (dispersed
performance) independent of the group's sync rank or performance breadth
reading. `</observation>`
<!-- src: belief/correlation-framework.md L282-288 -->

**Scope caveat:** `<out-of-scope>` Does not itself indicate whether the
dispersion is favorable or unfavorable — that interpretation is
regime-dependent and is trading-strategy content, not captured here (see
STR-003). `</out-of-scope>`
<!-- src: belief/correlation-framework.md L284-288 (regime-dependent
interpretation deliberately routed to STR-003, not dropped) -->

*(cross-refs: MB-006 — sync rank; MB-012 — performance breadth; distinct
from both — see glossary Performance dispersion; added 2026-07-07 per
trader review, splitting STR-003's prior MB-007 citation into a dedicated
phenomenon entry; numbered MB-023 rather than the vacated MB-022 — MB-022
was previously assigned (to "Theme Validity Persists...", since renumbered
to MB-020) and reusing it would be a second exception to the "IDs never
reused" rule beyond the single logged 2026-07-06 exception; see ID history
below)*
...
- MB-001, MB-002, MB-005–021, MB-023 (i.e. every current entry) Conditions fields
...
- v0.8 (2026-07-07) — trader-peer review session corrections: MB-001
  "activation day" → "spike day" (legacy term removal, "visually discrete"
  volume phrasing locked in); MB-006 Phenomenon/Observable indicators
  rewritten (all three sync types drive sync rank equally, not
  resemblance-primacy — corrected to match glossary Sync rank/Type 1
  amendments; decay/trajectory framing added), Example removed per Rule 16
  (no-silent-drop, provisional pending trader-supplied replacement); MB-009
  retitled and rewritten from a market-cap mechanism to the actual
  float/institutional-dollar-flow mechanism; MB-011 "intraday timeframe" →
  "30- to 60-minute timeframe" (both fields, removes tick/1-minute
  ambiguity); new entry MB-023 (Performance Dispersion Among Group Members),
  split out of trading-strategies.md STR-003's prior MB-007 citation since
  dispersion and performance breadth (MB-012) are distinct glossary
  concepts — numbered MB-023, not the vacated MB-022, to avoid a second
  "IDs never reused" exception.
